...
def remote_mount(c, workstation_dir, server_dir, queue=None):
    ...
    sshfs_cmd = ssh_cmd["sshfs", "-f", f"-p {port}", "-o allow_other,default_permissions,ro",
...
def local_mount(c, workstation_dir, server_dir, queue=None):
    ...
    sshfs_cmd = sshfs["-f", "-o", "allow_other,default_permissions,ro,StrictHostKeyChecking=no,reconnect", f"{c.user}@{c.host}:{server_dir}", workstation_dir]
